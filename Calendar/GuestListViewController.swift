...
class GuestListViewController: UITableViewController, EVContactsPickerDelegate {
    ...
    override func tableView(_: UITableView, accessoryButtonTappedForRowWith: IndexPath) {
        ...
        let record = accessoryButtonTappedForRowWith.section == 0 ? tableViewData?[accessoryButtonTappedForRowWith.row] as! NSDictionary : tableViewData?[accessoryButtonTappedForRowWith.row] as! NSDictionary
        let id = (record["identifier"] as! String)
        fetchDetails(id: id)
    }
