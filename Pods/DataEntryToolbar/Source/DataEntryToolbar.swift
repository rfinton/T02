    ...
    */
    private func findPreviousValidIndexPath(indexPath: NSIndexPath) -> NSIndexPath? {
        ...
        if indexPath.row > 0 {
            ...
            let previousIndexPath: NSIndexPath = NSIndexPath(row: indexPath.row - 1, section: indexPath.section)
            if let foundTextField: UITextField = self.tableTextFields[previousIndexPath] {
                foundTextField.becomeFirstResponder()
                return previousIndexPath
            } else {
                self.findPreviousValidIndexPath(indexPath: previousIndexPath)
            }
        } else {
            ...
            if indexPath.section > 0 {
                ...
                if let previousSectionRows = self.tableView?.numberOfRows(inSection: previousSection) {
                     let previousIndexPath = NSIndexPath(row: previousSectionRows - 1, section: previousSection)
        ...
        if let lastRowOfSectionIndex = self.tableView?.numberOfRows(inSection: indexPath.section) {
            if indexPath.row < lastRowOfSectionIndex - 1 {
                ...
                if let foundTextField: UITextField = self.tableTextFields[nextIndexPath] {
                    ...
                    self.findNextValidIndexPath(indexPath: nextIndexPath)
                }
            } else {
                ...
                if let lastTableSection = self.tableView?.numberOfSections {
                    ...
                    if nextTableSection < lastTableSection - 1 {
                         let nextIndexPath = NSIndexPath(row: 0, section: nextTableSection) 
    ...
    @objc private func previousButtonTapped() {
        ...
        for (indexPath, textField) in self.tableTextFields {
            ...
            if textField.isFirstResponder {
                ...
                if let prevPath: NSIndexPath = findPreviousValidIndexPath(indexPath: indexPath) {}
                break
            }
        }
    ...
    @objc private func nextButtonTapped() {
        ...
        for (indexPath, textField) in self.tableTextFields {
            ...
            if textField.isFirstResponder {
                ...
                if let nextPath: NSIndexPath = findNextValidIndexPath(indexPath: indexPath) {}
                break
            }
        }
